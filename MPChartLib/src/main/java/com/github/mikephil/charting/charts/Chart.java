...
public abstract class Chart<T extends ChartData<? extends IDataSet<? extends Entry>>> extends
    ...
    protected Queue<Highlight> mIndicesToHighlightQueue;
    ...
    protected void setLastHighlighted(Highlight[] highs) {
        ...
        if (highs == null || highs.length <= 0 || highs[0] == null) {
            mChartTouchListener.setLastHighlighted(null);
            mChartTouchListener.setmLastHighlightedQueue(null);
        } else {
            mChartTouchListener.setLastHighlighted(highs[0]);
            mChartTouchListener.setmLastHighlightedQueue(highs[0]);
        }
    }
    ...
    public void highlightValue(Highlight high, boolean callListener) {
        ...
        if (high == null)
            ...
            if (e == null) {
                ...
                if (mIndicesToHighlightQueue == null){
                    mIndicesToHighlightQueue = new LinkedList<Highlight>();
                }
                if (mIndicesToHighlightQueue.size() >= 2) {
                    mIndicesToHighlightQueue.poll();
                }

                mIndicesToHighlightQueue.offer(high);
            }
        }
